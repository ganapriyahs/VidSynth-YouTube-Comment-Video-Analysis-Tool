...
# set up consistent logging for this service
configure_logging()
log = get_logger("read_service")
...
def extract_video_id(url: str) -> str | None:
    ...
    try:
        ...
        log.error(f"Error parsing URL '{url}': {e}", exc_info=True)
        return None

@app.get("/")
def root():
    """Returns a simple message indicating the service is running."""
    log.info("Health check endpoint called - service is running.")
...
def read(request: ReadRequest):
    ...
    """
    log.info(f"READ: Received request for {request.video_link}")
    video_id = extract_video_id(request.video_link)

    if not video_id:
        log.warning(f"Could not extract video_id from: {request.video_link}")
        raise HTTPException(status_code=400, detail="Invalid YouTube video link provided.")

    log.info(f"READ: Extracted video_id: {video_id}")
    log.info("READ: Returning response successfully.")
