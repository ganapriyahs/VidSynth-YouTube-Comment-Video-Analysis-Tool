...
from youtube_client import youtube_client # anytime you import, you run entire module
import logging  
...
def preprocess(request: VideoIdInput):
    ...
    if not comments:
        logger.warning(f"Could not retrieve any data (just comments) for video_id: {video_id}")
        logger.info(f"PREPROCESS SERVICE: No comment data found for video_id: {video_id}, returning empty string.")
    else:
        logger.info(f"PREPROCESS SERVICE: Successfully fetched comment data for video_id: {video_id}")
